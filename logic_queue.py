...
class LogicQueue(object):
        ...
        try:
            ...
            for i in ModelQueue.get_list():
                ...
                download = APIYoutubeDL.download(
                    ...
                    format_code=i.format,
                    preferredcodec="mp3" if i.convert_mp3 else None,
                    dateafter=date_after,
                    playlist="reverse" if i.playlistreverse else None,
                    start=False,
                ...
                if download["errorCode"] == 0 and sub["errorCode"] == 0:
                    ...
                    logger.debug(
                        "queue add fail %s %s", download["errorCode"], sub["errorCode"]
                    )
                    i.delete()
            ...
            while not ModelQueue.is_empty():
                ...
                if start["errorCode"] == 0:
                    while True:
                        ...
                        status = APIYoutubeDL.status(
                            package_name, entity.index, entity.key
                        )
    ...
    def add_queue(url, options):
        try:
            ...
            date_after = (
                entity.date_after.strftime("%Y%m%d") if entity.date_after else None
            )
            download = APIYoutubeDL.download(
                package_name,
                entity.key,
