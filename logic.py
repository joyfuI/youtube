    ...
    def migration():
        try:
            ...
            if db_version < 2:
                ...
                cursor.execute(
                    "UPDATE youtube_setting SET value = ? WHERE key = 'default_save_path'",
                    (save_path,),
                )
                ...
                cursor.execute(
                    "UPDATE youtube_scheduler SET save_path = ?", (save_path,)
                )
            ...
            if db_version < 4:
                ...
                cursor.execute(
                    "ALTER TABLE youtube_scheduler ADD playlistreverse BOOLEAN"
                )
                cursor.execute(
                    "UPDATE youtube_scheduler SET playlistreverse = ?", (False,)
                )
